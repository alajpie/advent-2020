...
; nasm -f elf64 2b.asm && ld 2b.o -o 2b && ./2b
...
header:
  ...
  cmp byte [rsi+1], 45   ; '-'
  jne .first2            ; if the second character is not a dash, first position has two digits
.first1:
  movzx r8, byte [rsi]   ; only digit
  sub r8, 48             ; '0'
  add rsi, 2             ; move pointer to second
  jmp .second
.first2:
  ...
  sub r8, 48             ; '0'
  add rsi, 3             ; move pointer to second
.second:
  cmp byte [rsi+1], 32   ; ' '
  jne .second2           ; if the second character is not a space, second position has two digits
.second1:
...
.after:
  ...
  cmp byte [rsi+r8-1], r10b  ; r8 is one-indexed
  sete al
  cmp byte [rsi+r9-1], r10b  ; r9 is one-indexed
  sete bl
.skip:
  inc rsi
  cmp byte [rsi-1], 10         ; '\n'
  ...
  xor al, bl
  jz header                  ; xor is zero, don't increment
  inc rcx
  jmp header
